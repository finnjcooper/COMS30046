...
public:
	...
	uint8_t loadb(uint32_t addr) const { return data[addr]; }
	uint16_t loadh(uint32_t addr) const { return (data[addr]) | (data[addr + 1] << 8); }
	uint32_t loadw(uint32_t addr) const { return (data[addr]) | (data[addr + 1] << 8) | (data[addr + 2] << 16) | (data[addr + 3] << 24); }
